...
const localConfigPath = "/tmp/proj/.toby/config.json";
...
describe("config", () => {
	...
	describe("loadLocalConfig", () => {
		...
		it("returns parsed config when file exists", () => {
			mockConfigFile(localConfigPath, { specsDir: "docs" });
	...
	describe("loadConfig", () => {
		...
		it("local partial config merges with defaults", () => {
			mockConfigFile(localConfigPath, { verbose: true });
			const config = loadConfig("/tmp/proj");
		...
		it("config with transcript: true parses correctly", () => {
			mockConfigFile(localConfigPath, { transcript: true });
			const config = loadConfig("/tmp/proj");
	...
	describe("resolveCommandConfig", () => {
		...
		it("empty string model treated as default", () => {
			mockConfigFile(localConfigPath, { plan: { model: "" } });
			const config = loadConfig("/tmp/proj");
			const result = resolveCommandConfig(config, "plan");
